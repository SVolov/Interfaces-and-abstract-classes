...
namespace TaskFour
{
    public class Drone : IFlyable
    {
        ...
        public Coordinate CurrentPosition { get { return currentPosition; } set { currentPosition = new Coordinate(
            Math.Abs(value.X - currentPosition.X) <= 1000 ? value.X : 1000, value.Y,
            Math.Abs(value.Z - currentPosition.Z) <= 1000 ? value.Z : 1000); } }
        ...
        public double GetFlyTime(Coordinate newPosition)
        {
            ...
            if (newPosition.X - currentPosition.X <= 1000 && newPosition.Z - currentPosition.Z <= 1000)
            {
                ...
                time += timeHover;
                Console.WriteLine("Drone time = " + time);
                return time;
            }
